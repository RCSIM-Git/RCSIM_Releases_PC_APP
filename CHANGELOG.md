...
- Komponent bazowy (Core: sterowanie, kierownice FFB, FPV, telemetria, ELRS/CRSF) został odchudzony o ciężkie biblioteki PyTorcha, przyspieszając instalację i oszczędzając miejsce na dysku.
- Wdrożono architekturę *Graceful Degradation* w silniku wizji AI (`vision_engine.py`): przy braku modułu AI aplikacja uruchamia się w pełni stabilnie z obsługą klasycznej wizji OpenCV, a widgety informują o możliwości doinstalowania komponentu AI.
...
- Usunięto niezgodną bibliotekę ICU 78 z katalogu aplikacji; Windows używa teraz zgodnej biblioteki systemowej.
- Usunięto błąd punktu wejścia `UCNV_TO_U_CALLBACK_SUBSTITUTE` blokujący import `QtWidgets`.
...
- Przed sprawdzeniem CUDA aplikacja weryfikuje obecność sterownika NVIDIA (`nvcuda.dll`).
- Na komputerach bez karty NVIDIA moduły AI przechodzą bezpośrednio do trybu CPU, bez wywołania `torch.cuda.is_available()` powodującego błąd Access Violation podczas startu.
